...
# def log_api_call(current_program_idx, current_program_name, extraction_type, usage_data):
#     print(f"API Call Log: Program {current_program_idx} - {current_program_name}, Type: {extraction_type}, Usage: {usage_data}")
#     # Optionally, append to a log file
#     # with open("api_logs.txt", "a") as log_file:
#     #     log_file.write(f"{current_program_idx},{current_program_name},{extraction_type},{json.dumps(usage_data)}\n")

# def call_deepseek_with_cache(content: str, link: str, extraction_type: str = "unknown") -> dict:
...
#                         If content is ambiguous, leave it empty.
...
#                         Use these to convert degree abbreviations to full names and full names to degree_code.
#                         If the page provides the full name, use it and derive code from FULL_NAME_TO_CODE.
#                         If the page provides only an abbreviation, use ABBR_TO_FULL_NAME to get full name, and also normalize degree_code.
#                         ABBR_TO_FULL_NAME:
#                         {
...
#                         If the page only lists Major/Minor but not degree credential, degree_name and degree_code may be "".
...
#                         - If only a title is present without dates, store title and leave dates empty.
...
#                         If term/year is NOT explicitly stated in the same deadline context, do NOT guess year; leave date="".
...
#                         - Foundation core / Track prerequisites
...
#                         - overview: heading text for the block
...
#                         A) Prior education / eligibility => degreeRequirements
#                         - Examples: "Education level: Bachelor's", "Requires bachelor's degree"
#                         Output one entry:
#                         test.details: exact phrase or minimal faithful phrasing
#                         test.type: "" unless GPA/CGPA/Grade/Class explicitly stated
#                         test.total/outOf: 0 unless explicitly present
#                         test.region: "" unless explicitly present

#                         B) GPA / CGPA thresholds => degreeRequirements
#                         - Keep numeric exact; never infer outOf.
#                         test.type: "GPA" or "CGPA"
#                         test.total: threshold
#                         test.outOf: scale or 0

#                         C) Grade thresholds (B-, 2.75, etc.) => degreeRequirements
#                         - Store numeric value if present; letter grade stays in details.
#                         test.type: "Grade"
...
#                         E) English tests (TOEFL/IELTS/Duolingo) => languageRequirements
#                         - Split combined scores like 79/6.5/115 into separate entries.
#                         - Do not invent subscores.

#                         F) Documents & references => generalRequirements
#                         Allowed types ONLY:
...
#                         - Do NOT put them into degreeRequirements unless they are GPA/degree/grade.
...
#                         - If per credit hour is shown, DO NOT convert to per year. Leave tuitionFeePerYear empty; keep the link in usefulLink.
#                         - Extract ONLY if explicit numeric amounts exist in markdown.
#                         - Copy numbers EXACTLY as written (do not compute, do not convert).
#                         - currency: "USD" only if explicitly stated; otherwise "".
#                         - region: "International" / "Domestic" / “EU” / “Non-EU” only if explicitly stated; otherwise "".
...
#                         - title: step title or short phrase copied from markdown.
...
#                         usefulLink:
#                         - Extract only relevant links in a list: scholarship pages, requirements pages, tuition/cost info, financial aid, admissions, application/how-to-apply pages.
#                         - webLink must be an exact URL from markdown.
#                         - title should be short and copied from nearby anchor/heading text when possible.
#                         - detail MUST be copied verbatim from markdown(no regeneration).

#                         campusInfo:
#                         Extract only what appears explicitly in markdown; do not guess country

#                         scholarships.scholarships (FACTS ONLY)
#                         Rules:
#                         - Extract only explicitly stated scholarships, fellowships, assistantships, grants, or funding programs.
#                         - type: "Scholarship" | "Fellowship" | "Assistantship" | "Grant" | "Funding" | "Internal" | "External" (only if explicitly stated; else "").
#                         - name: exact name as written in markdown; else "".
...
#                                 "department": ""
#                             },
#                             "sessionDelivery": { "session": [{"title":"","startDate":"","endDate":"","recursive":""}], "durationYear": 0, "deliveryType": [] },

#                             "admissionTimeline": { "admissionTimeline": [{
#                         "type":"Open|Priority|Close",
#                         "title":"",
#                         "detail":"",
#                         "date":"",
#                         "session":"",
...
#                             "fees": { "tuitionFeePerYear": [{"amount":0 , "currency": "", "region": ""}], 

#                         "applicationFee": [{"amount":0, "currency": "", "region": ""}] },

#                             "howToApply": { "howToApply": [{"title":"","detail":"","webLink":""}] },
...
#                         }
#                         }] },
#                             "scholarships": { "scholarships": [{ "type":"", "name":"", "requirement":[],"detail":"","amount":0,"currency":"", "webLink":"","deadline":""
#                         }] },
#                             "usefulLink": [{"title": "","detail": "","webLink": ""}],
...
#                         If you find none, return one empty variant object.
...
#             log_api_call(current_program_idx, current_program_name, extraction_type, usage_data)
...
#     input_csv = "/home/ml-team/Desktop/BackupDisk/uniscrapupbackup/crawling-scrapping/latest_output_of_phase-2_scrape/extracted_programs_with_content_utulsa_latest_v2.csv"
#     df = pd.read_csv(input_csv)

#     for index, row in df.iterrows():
#         global current_program_idx, current_program_name

#         # Convert full row to dict so all columns are preserved
...
#         safe_name = quote(str(current_program_name), safe="")
#         output_filename = f"/home/ml-team/Desktop/BackupDisk/uniscrapupbackup/crawling-scrapping/deepseek-raw_output_utulsa_v2/{current_program_idx}_{safe_name}.json"
#         # output_filename = f"deepseek_raw_output_utulsa/{current_program_idx}_{current_program_name}.json"

#         with open(output_filename, "w", encoding="utf-8") as f:
#             json.dump(output_data, f, indent=4, ensure_ascii=False)

#         print(f"Processed program {current_program_idx}: {current_program_name}. Saved to {output_filename}")

#     print("Processing complete. Results saved to separate output_*.json files")
                ...
                messages=[{
                            ...
                            "content": """
                                        ...
                                        * "Close": Final deadline, Application closes, Deadline, Last date to apply, etc.
                                        ...
                                        - If the markdown mentions other standardized tests not in this list, do NOT include them in standardizedRequirements.
                                        - If "No"/"not required" => required=false and scores=0.
                                        - If no standardized test requirements exist in markdown, output empty array [].
                                        E) English tests (TOEFL/IELTS/Duolingo/PET/PTE) => languageRequirements
                                        - test.type: **FIXED VALUES ONLY** - MUST be one of: "TOEFL", "IELTS", "Duolingo", "PTE", or "PTE"
                                        - CRITICAL: Only use these exact five values. Do not include any other test types here.
                                        - Map any variations to these codes (e.g., "IELTS Academic" -> "IELTS", "Duolingo English Test" -> "Duolingo").
                                        - Split combined scores like 79/6.5/115 into separate entries.
                                        - Do not invent subscores.
                                        - If the markdown mentions other language tests not in this list, do NOT include them in languageRequirements.
                                        ...
                                        - Extract ONLY if explicit numeric amounts exist in markdown.
                                        - Copy numbers EXACTLY as written (do not compute, do not convert).
                                        - currency: **REQUIRED** - "USD" or if other country currency ISO stated, only if explicitly stated; otherwise ""
                                        - region: "International" / "Domestic" / "EU" / "Non-EU" /"In State"/"Out of State" only if explicitly stated; otherwise "".
                                        - Ignore salaries or unrelated monetary values.
                                        - For tuitionFeePerYear and applicationFee arrays, each object must have:
                                        ...
                                        - Extract only relevant links in a list: scholarship pages, requirements pages, tuition/cost info, financial aid, admissions, application/how-to-apply pages.
                                        - webLink: **REQUIRED** - must be an exact URL from markdown(can't be anything except link).If no URL found, do not create the entry.
                                        - title: **REQUIRED** - short and copied from nearby anchor/heading text when possible (never leave empty if link exists). Title can't be a link.
                                        - detail: MUST be copied verbatim from markdown (no regeneration).
                                        ...
                                        - name: **REQUIRED** - exact name as written in markdown; else "" (never leave empty if scholarship exists)
                                        - requirement: list of eligibility criteria only if explicitly listed; else [].
                                        - detail: MUST be copied verbatim from markdown (no regeneration).
                                        - amount: only if an explicit numeric amount appears; else 0.
                                        - currency: only if explicitly present; else "".
...
if __name__ == "__main__":
    input_csv = "/home/ml-team/Desktop/BackupDisk/uniscrapupbackup/crawling-scrapping/latest_output_of_phase-2_scrape/updated-extracted_programs_content_michiganu_latest_final_2.csv"
    output_dir = "/home/ml-team/Desktop/BackupDisk/uniscrapupbackup/crawling-scrapping/phase-3_output_final_program_data/updated_extracted_jsons_michiganu_latest_final/"

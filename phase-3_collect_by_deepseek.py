...
# ----------------------------- CACHING-OPTIMIZED API CALL -----------------------------
def call_deepseek_with_cache(content: str, instruction: str) -> dict:
    """
    Call DeepSeek API with caching optimization.
    Content is placed as system message (cached prefix - reused across extractions for SAME program).
    Instruction is placed as user message (varies per extraction).
    
    Cache isolation: Each unique content gets its own cache, expires after 5 mins of inactivity.
    This ensures one program's cache doesn't affect another program.
    """
    for attempt in range(5):
        try:
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": f"""You are an expert data extractor. You will be given program content to analyze.
Always return ONLY valid JSON objects as specified in the user's instruction.
...
# ----------------------------- EXTRACTION INSTRUCTIONS (NO CONTENT) -----------------------------

INSTRUCTION_PROGRAM_DEGREE = """Extract:
- program_name: Main program name (clean format, without degree abbreviation in parentheses). For example:
  * "Accounting (B.S.)" → "Accounting"
  * "Computer Science | Data Science emphasis" → "Computer Science | Data Science emphasis"
  * "Business Administration (MBA)" → "Business Administration"
  Extract the full program name with concentration/emphasis if present, but remove degree abbreviations in parentheses.
  
- degree_name: Full degree name from subheading or description, e.g., 'Master of Arts', 'Master of Science', 'Doctor of Philosophy', 'Bachelor of Science' etc.
  If degree abbreviation was in parentheses with program name (like "B.S.", "MBA", "M.S."), convert to full name.
  
If not found, use "".

Return ONLY: {"program_name": "", "degree_name": ""}"""

INSTRUCTION_SESSION_DURATION_DELIVERY = """IMPORTANT: 'session' is about CLASS START and END DATES (when the actual program/classes begin and end for each semester), NOT application deadlines.
...
INSTRUCTION_ADMISSION_TIMELINE = """IMPORTANT: 'admissionTimeline' is about APPLICATION/ADMISSION DEADLINES (when students need to apply), NOT class start dates.
...
INSTRUCTION_FEES = """Extract EXACT numeric values - do not regenerate or modify numbers:
- tuitionFeePerYear: List of {"amount": numeric value EXACTLY as stated, "currency": 'USD' or from content, "region": 'International' or from content}.
- applicationFee: Similar list with EXACT amounts.
...
INSTRUCTION_HOW_TO_APPLY = """Extract howToApply as list of dicts - one for each application step.
...
INSTRUCTION_ADMISSION_PROGRAM_TYPES = """Extract admissionProgramTypes as list of dicts.

IMPORTANT: Regenerate 'overview' in a student-friendly, conversational way. Use content as context but make it engaging and helpful.

Each dict:
- type: 'Major' or 'Minor' or whatever type is given from content or "" (program type).
- overview: REGENERATE this as a friendly, engaging overview that helps students understand the program. Use content as context but make it conversational, not too long (2-4 sentences).
- department: Department name of the specific program (e.g., 'Computer Science', 'Business', 'Engineering'). "" if not found.
- courseOutline: List of {"title": course/topic name, "detail": description of the course}.
...
INSTRUCTION_GENERAL_REQUIREMENTS = """Extract generalRequirements as list of dicts.
...
INSTRUCTION_STANDARDIZED_REQUIREMENTS = """Extract standardizedRequirements as list of dicts.
...
INSTRUCTION_LANGUAGE_REQUIREMENTS = """Extract languageRequirements as list of dicts.
...
INSTRUCTION_DEGREE_REQUIREMENTS = """Extract degreeRequirements as list of dicts.
...
INSTRUCTION_SCHOLARSHIPS = """Extract scholarships as list of dicts.
...
INSTRUCTION_USEFUL_LINKS = """Extract usefulLink as list of dicts.
...
Extract only relevant links: scholarship pages, requirements pages, tuition/cost info, financial aid, admissions, application, how to apply pages.
Empty list if none.

Return ONLY: {"usefulLink": []}"""

INSTRUCTION_CAMPUS_INFO = """Extract campus info from content (check Overview, Locations, Footer sections carefully):
- name: Campus names if specifically given. If multiple like 'Locations: Dhaka, Gulshan', concatenate as 'Dhaka, Gulshan'.
- webUrl: Base university URL (e.g., 'https://www.aiub.edu').
- email: Email address from Footer or Contact sections.
- phone: Phone number from Footer or Contact sections.
...
# ----------------------------- EXTRACTION FUNCTIONS (USE CACHING) -----------------------------

def extract_program_degree(content: str) -> Dict:
    return call_deepseek_with_cache(content, INSTRUCTION_PROGRAM_DEGREE)

def extract_session_duration_delivery(content: str) -> Dict:
    return call_deepseek_with_cache(content, INSTRUCTION_SESSION_DURATION_DELIVERY)

def extract_admission_timeline(content: str) -> Dict:
    return call_deepseek_with_cache(content, INSTRUCTION_ADMISSION_TIMELINE)

def extract_fees(content: str) -> Dict:
    return call_deepseek_with_cache(content, INSTRUCTION_FEES)

def extract_how_to_apply(content: str) -> Dict:
    return call_deepseek_with_cache(content, INSTRUCTION_HOW_TO_APPLY)

def extract_admission_program_types(content: str) -> Dict:
    return call_deepseek_with_cache(content, INSTRUCTION_ADMISSION_PROGRAM_TYPES)

def extract_general_requirements(content: str) -> Dict:
    return call_deepseek_with_cache(content, INSTRUCTION_GENERAL_REQUIREMENTS)

def extract_standardized_requirements(content: str) -> Dict:
    return call_deepseek_with_cache(content, INSTRUCTION_STANDARDIZED_REQUIREMENTS)

def extract_language_requirements(content: str) -> Dict:
    return call_deepseek_with_cache(content, INSTRUCTION_LANGUAGE_REQUIREMENTS)

def extract_degree_requirements(content: str) -> Dict:
    return call_deepseek_with_cache(content, INSTRUCTION_DEGREE_REQUIREMENTS)

def extract_scholarships(content: str) -> Dict:
    return call_deepseek_with_cache(content, INSTRUCTION_SCHOLARSHIPS)

def extract_useful_links(content: str) -> Dict:
    return call_deepseek_with_cache(content, INSTRUCTION_USEFUL_LINKS)

def extract_campus_info(content: str) -> Dict:
    return call_deepseek_with_cache(content, INSTRUCTION_CAMPUS_INFO)
...
def main():
    ...
    with open(CSV_INPUT, newline='', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        rows = list(reader)
    print(f"Processing {len(rows)} programs with separate prompts + caching optimization...\n")
    for idx, row in enumerate(rows, 1):
        # Clean row: replace None or 'nan' with ""
        for k in row:
            if row[k] is None or row[k] == 'nan':
        ...
        if chunks:
            print(f"[{idx}] Extracting from {len(chunks)} chunks (cache will be used per chunk)...")
            
            # ALWAYS extract program/degree for refinement (even if CSV has data)
            # This refines "Accounting (B.S.)" → program="Accounting", degree="Bachelor of Science"
            program_degree_results = run_extraction(extract_program_degree, chunks)
            program_name_vals = [r.get("program_name") for r in program_degree_results if r.get("program_name")]
            degree_name_vals = [r.get("degree_name") for r in program_degree_results if r.get("degree_name")]
            
            llm_program_name = program_name_vals[0] if program_name_vals else ""
            llm_degree_name = degree_name_vals[0] if degree_name_vals else ""
            
            # Use refined LLM values, fallback to CSV if LLM returns empty
            extracted["program_name"] = llm_program_name or csv_program_name
            extracted["degree_name"] = llm_degree_name or csv_degree_type
            
            if csv_program_name and llm_program_name and csv_program_name != llm_program_name:
                print(f"    ℹ Refined program_name: '{csv_program_name}' → '{llm_program_name}'")
            if csv_degree_type and llm_degree_name and csv_degree_type != llm_degree_name:
                print(f"    ℹ Refined degree_name: '{csv_degree_type}' → '{llm_degree_name}'")
            
            # Continue with other extractions (cache will be reused for same chunk)
            session_duration_results = run_extraction(extract_session_duration_delivery, chunks)
            extracted["durationYear"] = merge_extracted(session_duration_results, "durationYear", is_scalar=True)
            extracted["deliveryType"] = merge_extracted(session_duration_results, "deliveryType")
            extracted["session"] = merge_extracted(session_duration_results, "session", is_session=True)
            ...
            if need_campus_extraction:
                ...
                # Use extracted values (CSV fallback already handled above)
        final_program_name = extracted.get("program_name", "") or "Unknown Program"
        final_degree_name = extracted.get("degree_name", "") or ""
    ...
    print("All programs processed successfully with caching optimization!")

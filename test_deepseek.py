...
# # ----------------------------- CONFIG -----------------------------
# CSV_INPUT = "/home/ml-team/Desktop/BackupDisk/uniscrapupbackup/crawling-scrapping/test_deepseek.csv"
# OUTPUT_DIR = Path("extracted_structured_json_test_deepseek")
...
#     if not raw:
#         return ""
#     return re.sub(r'[\x00-\x08\x0b\x0c\x0e-\x1f\x7f-\x9f]', '', raw).strip()
...
# Extract the following fields from the content. Return ONLY valid JSON matching this exact structure.
...
#     for attempt in range(5):  # Retry up to 5 times on transient errors
#         try:
#             response = client.chat.completions.create(
#                 model="deepseek-chat",  # You can change to "deepseek-reasoner" if desired
...
#             wait = (2 ** attempt) * 10  # Exponential backoff: 10s, 20s, 40s, 80s, 160s
#             print(f"    Rate limited by DeepSeek. Waiting {wait} seconds before retry {attempt+1}/5...")
#             time.sleep(wait)
...
#     print(f"Processing {len(rows)} programs using DeepSeek...\n")
...
#         # Filename logic (with degree_type if present)
#         if degree_type:
...
#             if existing_files:
#                 numbers = [int(re.search(r"_extracted_(\d+)\.json$", f.name).group(1)) for f in existing_files if re.search(r"_extracted_(\d+)\.json$", f.name)]
...
#         print(f"[{idx}/{len(rows)}] Processing: {program_name} ({degree_type or 'No Degree'})")
...
#                 result = call_deepseek(chunk, program_name)
...
#             final_data = {}
#             defaults = ProgramDetails().model_dump()
#             for field, val in merged.items():
#                 final_data[field] = val if val not in [None, "", [], {}] else defaults[field]

#         # Save full row + extracted data
...
#     print("All programs processed successfully with DeepSeek!")
...
# # ----------------------------- CONFIG -----------------------------
# CSV_INPUT = "/home/ml-team/Desktop/BackupDisk/uniscrapupbackup/crawling-scrapping/test_deepseek_2.csv"
# OUTPUT_DIR = Path("/home/ml-team/Desktop/BackupDisk/uniscrapupbackup/crawling-scrapping/extracted_structured_json_test_deepseek_2")
# OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
...
#     if is_session:
#         merged = [{"title": "Spring", "startDate": "", "endDate": ""}, {"title": "Summer", "startDate": "", "endDate": ""}, {"title": "Fall", "startDate": "", "endDate": ""}]
#         for r in results:
#             for s in r.get(key, []):
#                 for m in merged:
#                     if m["title"] == s["title"]:
#                         m["startDate"] = s["startDate"] or m["startDate"]
#                         m["endDate"] = s["endDate"] or m["endDate"]
#         return merged
#     if is_list:
#         merged = []
#         seen = set()
...
# Extract:
# - program_name: Main program name(exact full program name only) from title or heading, e.g., 'Accountancy' ,'Computer Science' etc.
...
#     prompt = f"""You are an expert extractor. Return ONLY valid JSON object.
# Extract:
# - session: Always return the fixed list with titles 'Spring', 'Summer', 'Fall'. For each, set startDate to program start date for that session if mentioned (yyyy-mm-dd), endDate to end date if mentioned. Leave "" if not.
# - durationYear: Numeric years of program duration, e.g., 4 from '4 years'. 0 if not found.
# - deliveryType: List ['Online', 'Offline', 'Hybrid']. If physical locations like 'Moscow' mentioned, add 'Offline'. If online mentioned, 'Online'.

# Example: If content has 'Duration: 5 years', durationYear = 5.
# If specifically delivery type given then deliveryType = ['Offline'/'Online'/'Hybrid'] else "".
 
# Content: 
# {content}

# Return ONLY: {{"session": [{{"title": "Spring", "startDate": "", "endDate": ""}}, {{"title": "Summer", "startDate": "", "endDate": ""}}, {{"title": "Fall", "startDate": "", "endDate": ""}}], "durationYear": 0, "deliveryType": []}}"""
#     return call_deepseek(prompt)

# def extract_admission_timeline(content: str) -> Dict:
#     prompt = f"""You are an expert extractor. Return ONLY valid JSON object.
# Extract admissionTimeline(This is about the structured timeline of admission to the program with open,priority and close deadlines) as list of dicts.
# Each dict:
# - type: 'Open' for application opening, 'Priority' for priority deadlines, 'Close' for closing deadlines.
# - title: Short sentence about the timeline, e.g., 'Application Deadline'.
# - detail: Full detail from content.
# - date: Format as yyyy-mm-dd if date mentioned, else "".
# - session: 'Spring', 'Summer', 'Fall' if specified, else "".
# - region: 'International', 'Domestic', or country if specified, default 'International'.

# Example: If content has 'Application deadline: December 1', type='Close', date='current year-12-01' if no year, but use provided year.

# Empty list if no timeline info.
...
#     prompt = f"""You are an expert extractor. Return ONLY valid JSON object.
# Extract:
# - tuitionFeePerYear: List of {{"amount": numeric value, "currency": 'USD' or from content, "region": 'International' or from content}}.
# - applicationFee: Similar list.
...
#     prompt = f"""You are an expert extractor. Return ONLY valid JSON object.
# Extract howToApply(This is about all step by step procedure to apply for the program) as list of dicts.
# Each dict for a step of application process:
# - title: Step title or generated short sentence from content.
# - detail: Detailed description of the step.
# - webLink: Any link associated with the step, else "".

# If content has application info like 'Visit Graduate Admissions to learn more', make a step with title 'Visit Graduate Admissions', detail '...', webLink the url.
...
#     prompt = f"""You are an expert extractor. Return ONLY valid JSON object.
# Extract admissionProgramTypes(this is about the program and its associated courses outline for the program, each course will have one dict of title and detail) as list of dicts.
# Each dict:
# - type: 'Major' or 'Minor' from content or "" (Its about the program type).
# - overview: Brief overview from content about the program(regenerate it as more human friendly but not too large then actual one).
# - courseOutline: List of {{"title": a course section or a topic name of the program, "detail": description of the course}} from curriculum or related.
...
# Each dict:
# - type: One of the above list (each dict will contain one type only and all types should be included from the content).
# - title: Short note, e.g., 'it's abouyt the doc type'.
# - page: Page limit if mentioned (number, 0 if not).
# - words: Word limit if mentioned (number, 0 if not).
# - count: Number required, if mentioned of how many of the doc type is needed (number, 0 if not).
# - details: Full details about the doc type from content and regenerate it shortly as more human friendly tone.

# Example, For 'Number of references: 3', type='Recommendation Letter', title:'Academic reference required','page':0,words:0, count=3, details='every detail about this requirement'.

# Empty list if no matching.
...
# Each dict {{"test": {{"type": 'GRE' or 'GMAT' or 'SAT', "total": number or 0, "verbal": 0, "quant": 0, "awa": 0}}}}.
# For other tests, adapt fields.
# If 'GRE: No', skip as not required.

# Empty list if none or 'No'.
...
# Each dict {{"test": {{"type": 'TOEFL' or 'IELTS' or 'Duolingo', "total": number or 0, "listening": 0, "reading": 0, "writing": 0, "speaking": 0}}}}.
# For 'TOEFL/IELTS: 88/6.5', add two: TOEFL total=88, IELTS total=6.5.
...
# Each dict {{"test": {{"type": 'CGPA' or 'GPA' or 'Bachelor’s' or 'A Level', "total": number(if given) or 0, "outOf": number(if given) or 0, "region": string or ""}}}}.
# For 'GPA: 3.0', type='GPA', total=3.0, outOf=4.0 (skip if not specified).
# For 'Education level: Bachelor's', type='Bachelor’s', total=0.
...
# Each dict {{"type": 'Internal' (university) or 'External', "name": Scholarship name or 'Financial Aid', "requirement": [] list of requirements, "detail": full info, "amount": number or 0, "currency": 'USD' or ISO code of whatever currency given or "", "webLink": url or "", "deadline": 'yyyy-mm-dd' or ""}}.
# From 'Financial aid and scholarship opportunities' section, extract as 'Internal', detail the paragraph, webLink the financial aid url.
...
# Each dict {{"title": short description like 'Financial Aid Office', "webLink": the url}}.
# Extract only relevant links from content, e.g., scholarship page links, requirements page links, tuition fees/cost of attendance link, financial aid, admissions, how to apply page links.
...
#         filename = f"{safe_uni}_{safe_prog}_{safe_deg}_extracted.json" if final_degree_name else f"{safe_uni}_{safe_prog}_extracted.json"
#         output_file = OUTPUT_DIR / filename

#         if output_file.exists():
#             print(f"[{idx}/{len(rows)}] SKIP: {final_program_name} → Already exists")
#             continue
...
#                 {"title": "Spring", "startDate": "", "endDate": ""},
#                 {"title": "Summer", "startDate": "", "endDate": ""},
#                 {"title": "Fall", "startDate": "", "endDate": ""}
#             ]),
...
# # ----------------------------- NEW: EXTRACT SPECIFIC CAMPUS FIELD FROM SECTION -----------------------------
# def extract_specific_campus_field(section: str, field: str) -> str:
#     prompt = f"""You are an expert extractor. Return ONLY the value for {field}.
# From this section, extract {field} (e.g., for 'city': 'Moscow', for 'address': 'Full address string').
...
# # ----------------------------- NEW: EXTRACT UNIVERSITY WEBURL FROM PROGRAMURL -----------------------------
# def extract_university_weburl(program_url: str) -> str:
#     prompt = f"""You are an expert extractor. Return ONLY the base university website URL from this program URL: {program_url}.
# Example: from 'https://www.uidaho.edu/academics/degree-finder/accountancy-macct', base URL is 'https://www.uidaho.edu'.
# Return the base URL only if clear and confident, else ""."""
#     result = call_deepseek(prompt)
#     return result.get("webUrl", "")

# # ----------------------------- NEW: EXTRACT CAMPUS INFO -----------------------------
...
# - session: Always return the fixed list with titles 'Spring', 'Summer', 'Fall'. For each, set startDate to program start date for that session if mentioned (yyyy-mm-dd), endDate to end date if mentioned. Leave "" if not.
# - durationYear: Numeric years of program duration, e.g., 4 from '4 years'. 0 if not found.
# - deliveryType: List ['Online', 'Offline', 'Hybrid']. If specifically delivery type mentioned, add it to the list.

# Example: If content has 'Duration: 5 years', durationYear = 5.
# If specifically delivery type given then deliveryType = ['Offline'/'Online'/'Hybrid'] else empty deliveryType = [].
...
#     prompt = f"""You are an expert extractor. Return ONLY valid JSON object.
# Extract usefulLink as list of dicts.
# Each dict {{"title": short title like 'Graduate Admissions', "detail": short description like 'Official MIT graduate admissions page',"webLink": the url}}.
# Extract only relevant links from content, e.g., scholarship page links, requirements page links, tuition fees/cost of attendance link, financial aid, admissions, how to apply page links.
